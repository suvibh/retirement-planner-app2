...
try:
    ...
    st.error(f"🚨 Firestore Connection Failed: {e}"); st.stop()
...
def render_total(label, text): st.markdown(
...
with st.expander("💵 2. Active Annual Income Streams", expanded=False):
    st.markdown(
        '<div class="info-text">💡 <strong>Tax Torpedo Warning:</strong> The simulation separates Ordinary Income from Capital Gains. If you list a 401(k) withdrawal later, it increases your "Provisional Income," which can cause up to 85% of your Social Security to become taxable automatically.</div>',
        unsafe_allow_html=True)
    ...
    if df_inc.empty:
        ...
        for col in ["Override Growth (%)", "Start Age", "End Age"]:
            if col not in df_inc.columns: df_inc[col] = None if "Override" in col else my_age
...
with st.expander("🏢 3. Real Estate & Private Assets", expanded=False):
    ...
    if df_re.empty: df_re = pd.DataFrame(
        [{"Property Name": "Primary Home", "Is Primary Residence?": True, "Market Value ($)": 0,
          "Mortgage Balance ($)": 0, "Interest Rate (%)": 0.0, "Mortgage Payment ($)": 0, "Monthly Rent ($)": 0,
          "Monthly Expenses ($)": 0, "Override Prop Growth (%)": None, "Override Rent Growth (%)": None}])
    for c in ["Is Primary Residence?", "Mortgage Payment ($)", "Override Prop Growth (%)", "Override Rent Growth (%)"]:
        if c not in df_re.columns: df_re[c] = None if "Override" in c else (False if "Is Primary" in c else 0.0)
    ...
    edited_biz = st.data_editor(df_biz, num_rows="dynamic", width="stretch", hide_index=True, key="biz_editor")
...
with st.expander("🏦 4. Market Portfolios & Liabilities", expanded=False):
    ...
    if df_ast.empty: df_ast = pd.DataFrame(
        [{"Account Name": "401k", "Type": "Traditional 401k/IRA", "Owner": "Me", "Current Balance ($)": 0,
          "Annual Contribution ($)": 0, "Est. Annual Growth (%)": 7.0}])
    if "Annual Contribution ($)" not in df_ast.columns: df_ast.insert(4, "Annual Contribution ($)", 0)
    ...
    if df_debt.empty: df_debt = pd.DataFrame(
        [{"Debt Name": "Auto Loan", "Type": "Auto Loan", "Current Balance ($)": 0, "Interest Rate (%)": 0.0,
          "Monthly Payment ($)": 0}])
    edited_debt = st.data_editor(df_debt, num_rows="dynamic", width="stretch", hide_index=True, key="debt_editor")
...
h_ctx = f"Primary housing costs ${h_pmt + h_exp:,.0f}/mo." if (h_pmt + h_exp) > 0 else "Renting residence."
f_profile = f"User(DOB:{my_dob})" + (
    f", Spouse(DOB:{spouse_dob})" if has_spouse else "") + f", Kids({', '.join([f'{k['name']}:{k['age']}' for k in kids_data])})"

# --- 5. CURRENT EXPENSES ---
with st.expander("💸 5. AI Smart Budget Builder (Current)", expanded=False):
    st.markdown(
        '<div class="info-text">💡 <strong>How AI Estimation Works:</strong> We query Google Gemini with your Family Size, Ages, City, and Debts. It cross-references global consumer price indices (CPI) to hallucinate a hyper-realistic, localized budget.</div>',
        unsafe_allow_html=True)
    df_c = pd.DataFrame(st.session_state['current_expenses'])
    if df_c.empty: df_c = pd.DataFrame(
    ...
    edited_c = st.data_editor(
        ...
        column_config={
            "Category": st.column_config.SelectboxColumn("Category",
                                                         options=["Housing", "Transportation", "Food", "Utilities",
    ...
    if st.button("✨ Auto-Estimate Budget for " + (curr_city if curr_city else "Your Area") + " (AI)"):
        with st.spinner("Analyzing localized CPI data and family needs..."):
            ...
            prompt = f"City: {curr_city}. Family: {k_ctx} Housing: {h_ctx}. Generate missing items for a complete localized budget. Return ONLY JSON array. Locked: {json.dumps(locked)}"
            res = call_gemini_json(prompt)
            if res: st.session_state['current_expenses'] = locked + res; st.rerun()
...
with st.expander("🎉 6. AI Life Milestone Forecaster", expanded=False):
    ...
    if df_m.empty: df_m = pd.DataFrame(
    ...
    if st.button("✨ Forecast Milestone Timelines & Costs (AI)"):
        with st.spinner("AI mapping family timelines and projecting future costs..."):
            ...
            prompt = f"Family Context: {f_profile}. Current Date: {current_date_str}. Calculate Start/End dates (MM/YYYY) and future Amounts in today's dollars for: {json.dumps(valid)}. Return ONLY JSON array."
            res = call_gemini_json(prompt)
            if res: st.session_state['one_time_events'] = res; st.rerun()

# --- 7. RETIREMENT SCENARIO & ASSUMPTIONS ---
with st.expander("🔮 7. Global Macroeconomic Assumptions", expanded=False):
    st.markdown(
        '<div class="info-text">💡 <strong>Granular Inflation:</strong> We separate CPI, Healthcare, and Education inflation. This prevents groceries from hyper-inflating while properly modeling the rapid rise in medical costs.</div>',
        unsafe_allow_html=True)
    ...
    life_exp = c3.slider("Plan Life Expectancy", 70, 115, int(p_info.get('life_expectancy', 95)),
                         help="Most planners recommend 95 to ensure you don't outlive your money.")
    ...
    if st.button("✨ Auto-Set Macroeconomic Assumptions for " + (ret_city if ret_city else "Retirement") + " (AI)"):
        with st.spinner(f"Analyzing economic forecast and historical data for {ret_city}..."):
            ...
            res = call_gemini_json(prompt)
            if res: st.session_state['assumptions'].update(res); st.rerun()
    ...
    if st.button("✨ Simulate Realistic Lifestyle Costs in " + (ret_city if ret_city else "Retirement") + " (AI)"):
        with st.spinner(f"Modelling specific living costs for {ret_city}..."):
            ...
            prompt = f"Retirement context: {ret_city}. Household size drops to {1 + (1 if has_spouse else 0)}. Existing items: {json.dumps(user_rows)}. Generate realistic localized cost. Return JSON array."
            res = call_gemini_json(prompt)
            if res: st.session_state['retire_expenses'] = res; st.rerun()
...
with st.expander("📈 9. Advanced Simulation & Analytics Dashboard", expanded=True):
    if my_age > 0 and life_exp > my_age:
        ...
        ss_fra = 67 if my_dob.year >= 1960 else (
            66 + (min(my_dob.year - 1954, 10) / 12.0) if my_dob.year >= 1955 else 66)
        ss_multiplier = 1.0
        if ret_age < ss_fra:
            months_early = (ss_fra - ret_age) * 12
            if months_early <= 36:
                ss_multiplier = 1.0 - (months_early * (5 / 9 * 0.01))
            else:
                ss_multiplier = 1.0 - (36 * (5 / 9 * 0.01)) - ((months_early - 36) * (5 / 12 * 0.01))
        elif ret_age > ss_fra:
            months_late = min((ret_age - ss_fra) * 12, 36)
            ss_multiplier = 1.0 + (months_late * (2 / 3 * 0.01))
        ...
        # INIT STATE
        sim_assets = [{"Account Name": a.get("Account Name"), "Type": a.get("Type"),
        ...
        if not sim_assets: sim_assets = [
            {"Account Name": "Unallocated Cash", "Type": "Checking/Savings", "bal": 0.0, "contrib": 0.0, "growth": 0.0}]
        ...
        for age in range(int(my_age), int(life_exp) + 1):
            year = current_year + (age - my_age)
            is_retired = age >= int(ret_age)
            ...
            is_spouse_alive = True if has_spouse and (spouse_age + (age - my_age)) <= 95 else False
            active_mfj = True if has_spouse and is_spouse_alive else False
            ...
            for b in sim_biz:
                if age > int(my_age): b['val'] *= (1 + active_mkt / 100); b['dist'] *= (1 + inc_g / 100)
            ...
            for r in sim_re:
                if age > int(my_age): r['rent'] *= (1 + r['r_growth'] / 100); r['exp'] *= (1 + infl / 100); r[
            ...
            # LTC Shock
            if ltc_shock and age >= (int(life_exp) - 2):
                ltc_cost = 100000 * ((1 + infl_hc / 100) ** (age - my_age))
                total_exp += ltc_cost;
            ...
            for ev in edited_m.to_dict('records'):
                if ev.get("Description"):
                    try:
                        ...
                        sy = 0
                    if sy == year and sy != 0:
                        amt = safe_num(ev.get('Amount ($)')) * ((1 + infl / 100) ** (age - my_age))
                        if ev.get('Type') == 'Expense':
                            total_exp += amt; yd[f"Expense: Milestone ({ev.get('Description')})"] = amt
                        else:
                            annual_inc += amt; pre_tax_ord += amt; yd[
                                f"Income: Milestone ({ev.get('Description')})"] = amt

            # Asset Waterfall Routing
            liquid_assets_total, asset_contributions = 0, 0
            if not is_retired:
                for a in sim_assets: a['bal'] += a['contrib']; asset_contributions += a['contrib']
            ...
            # Tax Calculations
            base_fed_tax, marginal_rate = calc_federal_tax(pre_tax_ord, 0, active_mfj, (age - my_age), infl)
                ...
                if shortfall > 0:
                    for a in sim_assets:
                        ...
                        if a.get('Type') in ['Roth 401k/IRA', 'HSA', 'Crypto', '529 Plan', 'Other']:
                            if a['bal'] >= shortfall:
                                a['bal'] -= shortfall; shortfall = 0
                            else:
                                shortfall -= a['bal']; a['bal'] = 0
        ...
        if final_nw >= 1000000:
            st.success(
                f"🟢 **On Track:** Projected Net Worth at Age {life_exp} is **${final_nw:,.0f}**. Your assets outlive your life expectancy comfortably.")
        elif final_nw > 0:
            st.warning(
                f"🟡 **Caution:** Projected Net Worth at Age {life_exp} is **${final_nw:,.0f}**. You are solvent, but with a narrow margin of safety.")
        else:
            st.error(
        ...
        if HAS_PLOTLY:
            ...
            fig_nw.add_trace(go.Scatter(x=df_sim["Age"], y=df_sim["Real Estate Equity"], mode='lines', stackgroup='one',
                                        name='Real Estate Equity', fillcolor='rgba(139, 92, 246, 0.5)',
                                        line=dict(color='#8b5cf6')))
            fig_nw.add_trace(go.Scatter(x=df_sim["Age"], y=df_sim["Business Equity"], mode='lines', stackgroup='one',
                                        name='Business Equity', fillcolor='rgba(245, 158, 11, 0.5)',
                                        line=dict(color='#f59e0b')))
            fig_nw.add_trace(
                go.Scatter(x=df_sim["Age"], y=df_sim["Debt"], mode='lines', stackgroup='two', name='Debt Liabilities',
                           fillcolor='rgba(244, 63, 94, 0.5)', line=dict(color='#f43f5e')))
            fig_nw.add_trace(go.Scatter(x=df_sim["Age"], y=df_sim["Net Worth"], mode='lines', name='Total Net Worth',
    ...
    if st.session_state['user_email'] == "guest_demo":
        ...
        user_data = {
            "personal_info": {"name": my_name, "dob": my_dob.strftime("%Y-%m-%d"), "age": my_age, "retire_age": ret_age,
                              "spouse_retire_age": s_ret_age, "life_expectancy": life_exp, "current_city": curr_city,
